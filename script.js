document.addEventListener('DOMContentLoaded', function() {
  ...
  // Mobile menu toggle
  const mobileMenuToggle = document.querySelector('.mobile-menu-toggle, .hamburger, [data-menu-toggle]');
  const mobileMenu = document.querySelector('.mobile-menu, .nav-menu, [data-menu]');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מרפאת מחמד משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '143d8020-716b-4e9e-b4a2-81322771db1c',
                    name: data.name || '',
                    email: data.email || '',
